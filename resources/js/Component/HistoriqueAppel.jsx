...
const HistoriqueAppel = () => {
  ...
  const fetchResumer = async () => {
    try {
      ...
      const appelsOnly = response.data.filter(resumer => resumer.source === 'appel');
      setResumer(appelsOnly);
      setFilteredAppels(appelsOnly);
    } catch (error) {
        ...
        <div className="card-body">
          ...
          <div className="table-responsive mt-4">
            <table className="table table-striped">
              ...
              <tbody>
                {currentAppels.map(appel => (
                  <tr key={appel.id}>
                    <td>{appel.type_source}</td>
                    <td>{appel.entreprise}</td>
                    <td>{appel.date_resumer}</td>
                  </tr>
                ))}
              </tbody>
            </table>
