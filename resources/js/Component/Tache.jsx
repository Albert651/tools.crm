  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'add') {
        const taskToSubmit = {
          ...newTache,
          Intervenant: isChecked ? newTache.Intervenant : 'Tâche non effectuée', // Adjust based on checkbox
          etat: isChecked ? 'En cours' : 'À faire', // Set appropriate state
        };
  
        const response = await axios.post('/Taches', taskToSubmit);
        setAlert({ show: true, message: 'Tâche ajoutée avec succès.', variant: 'success' });
        const addedTask = response.data; // Supposons que l'API renvoie la tâche ajoutée
        const updatedNewTasks = [...newTasks, addedTask];
        setNewTasks(updatedNewTasks);
        onNewTaskAdded(updatedNewTasks); // Notifier le parent avec toutes les nouvelles tâches
      } else {
